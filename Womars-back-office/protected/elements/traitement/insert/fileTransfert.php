...
<?php
    ...
    if (!empty($_FILES["fileToUpload"]["tmp_name"])){
        ...
        if (isset($_POST["submit"])) {
            ...
            else { $uploadOk = 0; header("location:../../pages/back-office.php?page=Media&errorMedia=error7"); exit(); }
        }

        if (file_exists($target_file)) {
            $uploadOk = 0;
            header("location:../../pages/back-office.php?page=Media&errorMedia=error3");
            exit();
        }

        if ($_FILES["fileToUpload"]["size"] > 500000) {
            $uploadOk = 0;
            header("location:../../pages/back-office.php?page=Media&errorMedia=error4");
            exit();
        }

        if ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif" ) {
            $uploadOk = 0;
            header("location:../../pages/back-office.php?page=Media&errorMedia=error5");
            exit();
        }

        if ($uploadOk === 0) {
            header("location:../../pages/back-office.php?page=Media&errorMedia=error6");
            exit();
        } else {
            if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                $nullConst = null;
                $sql = $db->prepare("INSERT INTO images (name, size, type, caption, alt) VALUES (:name, :size, :type, :caption, :alt)");
