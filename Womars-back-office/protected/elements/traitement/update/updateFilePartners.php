...
<?php
    if(isset($_POST['update'])){
        $sql = $db->prepare("UPDATE partners SET name=:name, link=:link, caption=:caption, alt=:alt WHERE id=:id");
        $sql->bindParam(':name', $_POST["name"]);
        if ($_POST['link'] !== ''){ $sql->bindParam(':link', $_POST["link"]); }
        else { $sql->bindValue(':link', null, PDO::PARAM_NULL); }
